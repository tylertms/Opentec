...
static void test_inactive_and_unchanged(void) {
    ...
    assert(!motor_center_command_apply(&state, 11, 100, 0U, 50U, &encoder_offset));
    assert(state.requested == 10);

    state.active = true;
    assert(!motor_center_command_apply(&state, 10, 100, 0U, 50U, &encoder_offset));
}

static void test_offset_clamp(void) {
    MotorCenterState positive = {.active = true};
    int32_t positive_offset = 101;
    assert(motor_center_command_apply(&positive, 1, 100, 0U, 50U, &positive_offset));
    assert(positive_offset == 100);

    MotorCenterState negative = {.active = true};
    int32_t negative_offset = -101;
    assert(motor_center_command_apply(&negative, 1, 100, 100U, 50U, &negative_offset));
...
static void test_shared_endpoint(void) {
    ...
    assert(motor_center_command_apply(&negative, 1, 100, 49U, 50U, &negative_offset));
    assert(negative_offset == 0);

    MotorCenterState positive = {.active = true};
    int32_t positive_offset = 100;
    assert(motor_center_command_apply(&positive, 1, 100, 50U, 50U, &positive_offset));
