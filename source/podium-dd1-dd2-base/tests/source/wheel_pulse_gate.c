...
static void test_packed_modes_use_eighty_millisecond_deadlines(void) {
    WheelPulseGate gate;
    wheel_pulse_gate_init(&gate);

    assert(wheel_pulse_gate_ready_for_packed(&gate, 6, 1, 0x10));
    assert(!wheel_pulse_gate_ready_for_packed(&gate, 6, 81, 0x10));
    assert(wheel_pulse_gate_ready_for_packed(&gate, 6, 82, 0x10));

    wheel_pulse_gate_init(&gate);
    assert(wheel_pulse_gate_ready_for_packed(&gate, 7, 1, 0x10));
    assert(!wheel_pulse_gate_ready_for_packed(&gate, 7, 81, 0x10));
    assert(wheel_pulse_gate_ready_for_packed(&gate, 7, 82, 0x10));
}

int main(void) {
    test_applies_independent_interface_deadlines();
    test_zero_flags_do_not_extend_a_deadline();
    test_packed_modes_use_eighty_millisecond_deadlines();
    return 0;
}
