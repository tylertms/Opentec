...
static void test_filters_buttons_and_axes_across_three_samples(void) {
    WheelPacketCommonFilter filter;
    WheelPacketAxisModeInput input = {0};
    wheel_packet_common_filter_init(&filter);
    ...
    input.controls[4] = 30;
    input.controls[5] = 60;
    wheel_packet_common_filter(&filter, &input);
...
static void test_expands_packed_output_nibbles(void) {
    ...
    wheel_packet_common_expand_packed_controls(&input);
