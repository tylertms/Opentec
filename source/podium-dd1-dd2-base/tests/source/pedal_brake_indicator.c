...
static void test_activates_at_scaled_threshold(void) {
    ...
    assert(pedal_brake_indicator_update(&indicator, 50, UINT16_C(0x7fff), false) == UINT8_MAX);
    assert(pedal_brake_indicator_update(&indicator, 50, UINT16_C(0xffff), false) == UINT8_MAX);
}
...
static void test_selects_legacy_transport(void) {
    ...
    assert(pedal_brake_indicator_update(&indicator, 100, UINT16_MAX, true) == 0x3f);
}
...
static void test_releases_once_below_threshold(void) {
    ...
    assert(pedal_brake_indicator_update(&indicator, 1, UINT16_C(0x02ff), false) == UINT8_MAX);
...
static void test_disables_level_101(void) {
    ...
    assert(pedal_brake_indicator_update(&indicator, 0, 0, false) == UINT8_MAX);
