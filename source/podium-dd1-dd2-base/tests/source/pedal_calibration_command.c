...
enum {
    AUXILIARY_SOURCE_DISCONNECTED_SAMPLE = 61,
    AUXILIARY_SOURCE_CONNECTED_SAMPLE = 62,
};
...
static void test_routes_control_and_current_auxiliary_sample_independently(void) {
    PedalCalibrationCommand command;
    UsbOperatingModeCommand source = source_command(0x11, 0, 0, 0);
    assert(pedal_calibration_command_decode(&source, &command));

    PedalCalibrationActions actions =
        pedal_calibration_command_route(&command, true, AUXILIARY_SOURCE_CONNECTED_SAMPLE);
    assert(actions.pedal_control == PEDAL_V3_CONTROL_UP);
    assert(actions.auxiliary_action == PEDAL_AUXILIARY_CALIBRATION_MAXIMUM);

    actions = pedal_calibration_command_route(&command, false, AUXILIARY_SOURCE_CONNECTED_SAMPLE);
    assert(actions.pedal_control == 0);
    assert(actions.auxiliary_action == PEDAL_AUXILIARY_CALIBRATION_MAXIMUM);

    source.parameters[0] = 0x12;
    assert(pedal_calibration_command_decode(&source, &command));
    actions = pedal_calibration_command_route(&command, true, AUXILIARY_SOURCE_DISCONNECTED_SAMPLE);
...
static void test_routes_enable_and_disable_to_auxiliary_reset(void) {
    ...
    assert(pedal_calibration_command_decode(&source, &command));
    actions = pedal_calibration_command_route(&command, true, AUXILIARY_SOURCE_CONNECTED_SAMPLE);
    assert(actions.pedal_control == PEDAL_V3_CONTROL_DISABLE);
    assert(actions.auxiliary_action == PEDAL_AUXILIARY_CALIBRATION_RESET);
}

static void test_probes_current_auxiliary_presence_for_each_selector(void) {
    static const PedalCalibrationCommandKind kinds[] = {
    ...
    static const PedalAuxiliaryCalibrationAction expected[] = {
    ...
    for (uint8_t index = 0; index < sizeof(kinds) / sizeof(kinds[0]); index++) {
        PedalCalibrationCommand command = {.kind = kinds[index]};
        PedalCalibrationActions actions =
            pedal_calibration_command_route(&command, false, AUXILIARY_SOURCE_CONNECTED_SAMPLE);
        assert(actions.auxiliary_action == expected[index]);

        actions =
            pedal_calibration_command_route(&command, false, AUXILIARY_SOURCE_DISCONNECTED_SAMPLE);
        assert(actions.auxiliary_action == PEDAL_AUXILIARY_CALIBRATION_NONE);
    }
...
static void test_forwards_other_input_payloads_only_during_pedal_calibration(void) {
    ...
    actions = pedal_calibration_command_route(&command, false, AUXILIARY_SOURCE_CONNECTED_SAMPLE);
    assert(!actions.pedal_input_pending);
    actions = pedal_calibration_command_route(NULL, true, AUXILIARY_SOURCE_CONNECTED_SAMPLE);
    assert(!actions.pedal_input_pending);
}
...
int main(void) {
    ...
    test_routes_control_and_current_auxiliary_sample_independently();
    test_routes_enable_and_disable_to_auxiliary_reset();
    test_probes_current_auxiliary_presence_for_each_selector();
