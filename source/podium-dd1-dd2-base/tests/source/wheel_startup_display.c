...
static void test_waits_for_an_active_wheel(void) {
    ...
    assert(!wheel_startup_display_update(&display, false, false, true, 0, false, 100, &output));
...
static void test_presents_base_version_and_ready_delay(void) {
    ...
    assert(wheel_startup_display_update(&display, true, false, true, 0, false, 2, &output));
    assert(output.glyphs[0] == 0xcf);
    assert(output.glyphs[1] == 0xef);
    assert(output.glyphs[2] == 0x06);
    assert(!wheel_startup_display_update(&display, true, false, true, 0, false, 1000, &output));
    ...
    assert(!wheel_startup_display_take_extended_mode_clear(&display));
...
static void test_presents_managed_motor_version(void) {
    ...
    assert(wheel_startup_display_update(&display, true, false, true, &motor, false, 1, &output));
    assert(wheel_startup_display_update(&display, true, false, true, &motor, false, 1001, &output));
    assert(display.phase == WHEEL_STARTUP_DISPLAY_MOTOR_VERSION);
    assert(display.deadline_ms == 2001);
    assert(wheel_startup_display_update(&display, true, false, true, &motor, false, 1002, &output));
    assert(output.glyphs[0] == 0xb9);
    assert(output.glyphs[1] == 0xf7);
    assert(output.glyphs[2] == 0x71);
    assert(
        !wheel_startup_display_update(&display, true, false, true, &motor, false, 2001, &output));
    assert(
        !wheel_startup_display_update(&display, true, false, true, &motor, false, 2002, &output));
    assert(display.phase == WHEEL_STARTUP_DISPLAY_READY_DELAY);
...
static void test_reserves_longer_version_time_for_tuning_displays(void) {
    ...
    assert(wheel_startup_display_update(&display, true, true, true, 0, false, 1, &output));
    assert(display.deadline_ms == 3001);
    assert(wheel_startup_display_take_version_presentation(&display));
    assert(!wheel_startup_display_take_version_presentation(&display));
    assert(!wheel_startup_display_update(&display, true, true, true, 0, false, 2, &output));
    assert(output.glyphs[0] == 0x40);
    assert(!wheel_startup_display_update(&display, true, true, true, 0, false, 3000, &output));
    assert(!wheel_startup_display_update(&display, true, true, true, 0, false, 3001, &output));
    assert(display.phase == WHEEL_STARTUP_DISPLAY_READY_DELAY);
    assert(display.deadline_ms == 4001);
    assert(!wheel_startup_display_update(&display, true, true, true, 0, false, 3501, &output));
...
static void test_requests_extended_mode_clear_once_at_base_version_expiry(void) {
    ...
    assert(wheel_startup_display_update(&display, true, false, true, 0, true, 1, &output));
    assert(wheel_startup_display_update(&display, true, false, true, 0, true, 2, &output));
    assert(!wheel_startup_display_take_extended_mode_clear(&display));
    assert(!wheel_startup_display_update(&display, true, false, true, 0, true, 1000, &output));
    assert(!wheel_startup_display_take_extended_mode_clear(&display));
    assert(!wheel_startup_display_update(&display, true, false, true, 0, true, 1001, &output));
    assert(wheel_startup_display_take_extended_mode_clear(&display));
    assert(!wheel_startup_display_take_extended_mode_clear(&display));
}
...
static void test_blinks_calibration_until_position_is_ready(void) {
    ...
    assert(wheel_startup_display_update(&display, true, false, false, 0, false, 1, &output));
    assert(wheel_startup_display_update(&display, true, false, false, 0, false, 1001, &output));
    assert(display.phase == WHEEL_STARTUP_DISPLAY_CALIBRATION);
    assert(display.deadline_ms == 2001);

    assert(wheel_startup_display_update(&display, true, false, false, 0, false, 1002, &output));
    assert(output.glyphs[0] == 0x39);
    assert(output.glyphs[1] == 0x77);
    assert(output.glyphs[2] == 0x38);
    assert(!wheel_startup_display_update(&display, true, false, false, 0, false, 2001, &output));
    assert(wheel_startup_display_update(&display, true, false, false, 0, false, 2002, &output));
    assert(display.phase == WHEEL_STARTUP_DISPLAY_CALIBRATION_PAUSE);
    assert(display.deadline_ms == 2502);
    assert(output.glyphs[0] == 0);

    assert(!wheel_startup_display_update(&display, true, false, false, 0, false, 2502, &output));
    assert(!wheel_startup_display_update(&display, true, false, false, 0, false, 2503, &output));
    assert(display.phase == WHEEL_STARTUP_DISPLAY_CALIBRATION);
    assert(display.deadline_ms == 3003);
    assert(wheel_startup_display_update(&display, true, false, false, 0, false, 2504, &output));
...
int main(void) {
    ...
    test_requests_extended_mode_clear_once_at_base_version_expiry();
