...
static void test_repeats_digest_at_deadline(void) {
    ...
    assert(usb_xbox_gip_discovery_poll(&discovery, 0, 601) == USB_XBOX_GIP_DISCOVERY_DIGEST);
    assert(discovery.deadline == 1101);
}
...
int main(void) {
    test_repeats_digest_at_deadline();
