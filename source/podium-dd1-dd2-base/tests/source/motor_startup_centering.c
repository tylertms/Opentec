...
static void test_waits_for_controller_readiness(void) {
    ...
    reset_bus();
    motor_startup_centering_init(&centering, 100);
...
static void test_retries_failed_readiness_reads(void) {
    MotorStartupCentering centering;
    reset_bus();
    motor_startup_centering_init(&centering, 0);
...
static void test_readiness_timeout_is_exact_and_wrap_safe(void) {
    MotorStartupCentering centering;
    reset_bus();
    motor_startup_centering_init(&centering, UINT32_MAX - 2500);
...
static void test_failed_reads_do_not_block_startup_completion(void) {
    MotorStartupCentering centering;
    reset_bus();
    motor_startup_centering_init(&centering, 100);
    ...
    finish_read(0, PLATFORM_AUX_BUS_FAILED);
    assert(motor_startup_centering_run(&centering, 5099, false, 0) == 0);
    assert(!motor_startup_centering_complete(&centering));
    finish_read(0, PLATFORM_AUX_BUS_FAILED);
    assert(motor_startup_centering_run(&centering, 5100, false, 0) == 0);
    assert(motor_startup_centering_complete(&centering));
}
...
int main(void) {
    ...
    test_failed_reads_do_not_block_startup_completion();
    return 0;
}
