...
static void test_calibration_stage_and_completion(void) {
    ...
    ShifterDisplay display;
    WheelDisplayOutput output = {.glyphs = {0x12, 0x34, 0x56}};
    shifter_display_init(&display);
    ...
    assert(output.glyphs[0] == 0x7d);
    assert(output.glyphs[1] == 0x08);
    assert(output.glyphs[2] == 0x56);
    ...
    for (HPatternCalibrationPosition position = H_PATTERN_CALIBRATION_NEUTRAL;
        ...
        assert(output.glyphs[0] == 0x7d);
        assert(output.glyphs[1] == 0x08);
        assert(output.glyphs[2] == expected[position]);
    }

    assert(!shifter_display_update(&display, SHIFTER_GEAR_NEUTRAL, true,
                                   H_PATTERN_CALIBRATION_PROMPT_NONE,
                                   H_PATTERN_CALIBRATION_COMPLETE, 100, &output));
    assert(output.glyphs[2] == 0x07);
    assert(!shifter_display_update(&display, SHIFTER_GEAR_NEUTRAL, true,
                                   H_PATTERN_CALIBRATION_PROMPT_NONE,
                                   H_PATTERN_CALIBRATION_COMPLETE, 1100, &output));
    assert(shifter_display_update(&display, SHIFTER_GEAR_NEUTRAL, true,
...
static void test_calibration_prompt_preserves_official_page_slots(void) {
    ShifterDisplay display;
    ...
    assert(shifter_display_update(&display, SHIFTER_GEAR_NEUTRAL, true,
                                  ...
                                  H_PATTERN_CALIBRATION_REVERSE, 3, &output));
    assert(output.glyphs[0] == 0x7d);
    assert(output.glyphs[1] == 0x08);
    assert(output.glyphs[2] == 0x50);
}
...
int main(void) {
    ...
    test_calibration_prompt_preserves_official_page_slots();
