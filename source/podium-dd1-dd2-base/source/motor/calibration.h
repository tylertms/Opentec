...
typedef enum {
    MOTOR_CALIBRATION_IDLE, /**< No operation is currently being transferred; queued requests may
                               remain. */
    MOTOR_CALIBRATION_READ_IDLE,     /**< Read the motor calibration idle state before starting. */
    MOTOR_CALIBRATION_WRITE_COMMAND, /**< The calibration or erase command is being written. */
    MOTOR_CALIBRATION_READ_RESPONSE, /**< Poll the controller for the calibration result. */
...
typedef enum {
    ...
    MOTOR_CALIBRATION_EVENT_DISCONNECT_WHEEL =
        8, /**< Calibration was rejected because the wheel must be disconnected. */
    MOTOR_CALIBRATION_EVENT_UNSUPPORTED =
        9, /**< The operation was rejected because the accessory type is unavailable. */
    ...
    MOTOR_CALIBRATION_EVENT_STARTED =
        0x1c, /**< Calibration started after its request passed validation. */
...
/**
 * @brief Tests whether a motor-calibration exchange is actively using the controller.
 *
 * @param[in] service Motor-calibration service state to inspect.
 * @return True after queued work enters an exchange phase; otherwise false.
 */
