...
typedef struct {
    ...
    uint8_t *application_data;          /**< Dedicated storage for a retained motor response. */
...
typedef struct {
    ...
    const uint8_t *pending_payload; /**< Completed segmented command waiting for channel space. */
    uint16_t pending_payload_length; /**< Length of pending_payload in bytes. */
 ...
 * resets the USB upload and response state. Upload and response storage must be separate from the
 * channel's pending-payload storage so a deferred command or active response cannot be overwritten.
 *
 ...
 * false. Upload and response storage must not alias each other or the channel's pending-payload
 * storage.
 */
 ...
 * write actions. Requests are claimed but held while a response is active. A completed segmented
 * command remains pending when the motor channel is busy and is retried by the next service operation.
 *
 ...
 * action generated by the channel. Forwarded application responses are ignored while an earlier
 * response remains active so the retained response remains the sole owner of response storage.
 *
 * @param[in,out] service Active motor vendor service.
 ...
 * motor write, USB response, status, or mailbox action produced by the exchange. A deferred upload
 * waits until any active USB response has completed and the channel can accept it.
 *
 * @param[in,out] service Active motor vendor service.
